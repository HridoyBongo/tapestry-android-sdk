    ...
    public List<TypedIdentifier> get(Context context) {
        ...
        if (androidId != null) {
            try {
                ...
                Logging.e("Error hashing ANDROID_ID - MD5 not supported", nsae);
            }
            try {
                ids.add(new TypedIdentifier(TypedIdentifier.TYPE_ANDROID_ID_SHA1, DigestUtil.sha1Hash(androidId)));
            }
            catch (NoSuchAlgorithmException nsae) {
                Logging.e("Error hashing ANDROID_ID - SHA1 not supported", nsae);
            }
        }
        else {
            Logging.w("Error retrieving ANDROID_ID.");
        }
        return (ids);
    }
}
